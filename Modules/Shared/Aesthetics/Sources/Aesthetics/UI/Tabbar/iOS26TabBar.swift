...
struct iOS26TabBar<SelectionValue: Hashable>: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let tabBarHeight = 56 * scale
            let size = geo.size
            let visibleTabs = visibleTabs
            let tabsCount = CGFloat(max(visibleTabs.count, 1))
            let tabItemWidth = max(min(size.width / tabsCount, 90 * scale), 60 * scale)
            let tabItemHeight: CGFloat = tabBarHeight
                    ...
                    mainLayout {
                        ...
                        tabLayout {
                            ForEach(visibleTabs) { tab in
                                tabItemView(
                                    tab,
                                    width: isSearchExpanded ? 45 * scale : tabItemWidth,
                                    height: isSearchExpanded ? 45 * scale : tabItemHeight,
                                        ...
                                        ? ((
                                            activeTab == tab.value
                                                || (!visibleTabs.contains(where: {
                                                    $0.value == activeTab
                                                }) && lastVisibleSelection == tab.value)
                                        ) ? 1 : 0) : 1
                                )
                            }
                        }
        ...
        .frame(height: 56 * screenScale)
        .padding(.horizontal, isSearchExpanded
            ? (isKeyboardActive ? 10 * screenScale : 25 * screenScale)
            : 21 * screenScale)
        .padding(.bottom, isKeyboardActive ? 10 * screenScale : 0)
        .animation(bouncyAnimation, value: animationState)
    }

    // MARK: - Helpers

    /// Continuous scale factor relative to a 390pt-wide iPhone 14.
    private var screenScale: CGFloat {
        let w = UIScreen.main.bounds.width
        return ResponsiveLayout.DeviceSizeClass(width: w).scaleFactor(for: w)
    }
    ...
    private func tabBarBackground() -> some View {
        ZStack {
            Capsule(style: .continuous)
                .stroke(Color.cisumChromeBorder, lineWidth: 1.5)
            Capsule(style: .continuous)
    ...
    @ViewBuilder
    private func expandableSearchBar(height: CGFloat, scale: CGFloat) -> some View {
        let searchLayout =
            isKeyboardActive
        ...
        searchLayout {
            ...
            .padding(.horizontal, isSearchExpanded ? 15 * scale : 0)
            .padding(isSearchExpanded ? 0 : 3 * scale)
            .background(tabBarBackground())
            .geometryGroup()
            .zIndex(1)
