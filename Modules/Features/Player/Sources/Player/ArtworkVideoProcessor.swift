...
public actor ArtworkVideoProcessor {
    ...
    public enum ArtworkVideoProcessorError: LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case let .invalidInput(url):
                "Invalid HLS input URL: \(url.absoluteString)"
            case .cachePathFailure:
                "Unable to access the artwork video cache directory."
            case let .ffmpegFailure(message):
                message
            case .cancelled:
                "Artwork video processing was cancelled."
            case .noOutputProduced:
                "FFmpeg finished without producing an MP4 output file."
            }
        }
    }
    ...
    private enum FFmpegExecutionOutcome {
        case success
        case cancelled
        case failure(String)
    }
    ...
    public func prepareVideo(
        ...
        if let existingTask = inFlight[resolvedCacheID] {
            ...
            task = Task(priority: .utility) {
                let spid = artworkSP.begin("process-artwork", "id=\(mediaID)")
                defer { artworkSP.end("process-artwork", state: spid, "id=\(mediaID)") }

                return try await self.processVideo(
                    for: mediaID,
    ...
    private func processVideo(
        ...
        do {
            ...
            let sizeText = if let videoSize = sourceMetadata.videoSize {
            ...
            artworkSP.end("encode-artwork", state: encodeSpid, "id=\(mediaID)")
    ...
    private nonisolated func makeProgressScheduler(for mediaID: String) -> @Sendable (Double) -> Void {
        { [weak self] progress in
            guard let self else { return }
            scheduleProgressUpdate(progress, for: mediaID)
        }
    }

    private nonisolated func scheduleProgressUpdate(_ progress: Double, for mediaID: String) {
        Task(priority: .utility) { [weak self] in
            guard let self else { return }
            await notifyProgress(progress, for: mediaID)
        }
    }
    ...
    private func makeCommand(
        ...
        let codecArguments: [String] = switch strategy {
        case .hardwareTranscode:
            hardwareCodecArguments(for: sourceSize)
        case .softwareFallback:
            softwareCodecArguments(for: sourceSize)
        }
    ...
    private func loadSourceMetadata(from sourceHLSURL: URL) async -> SourceMetadata {
        ...
        do {
            ...
            let durationValue: Double? = if seconds.isFinite, !seconds.isNaN, seconds > 0 {
                seconds
            } else {
                nil
            }

            return await SourceMetadata(
                duration: durationValue,
                videoSize: largestVideoSize(from: loadedTracks)
            )
        } catch {
    ...
    private func shouldRetryWithFallback(for error: ArtworkVideoProcessorError) -> Bool {
        guard case let .ffmpegFailure(message) = error else {
            return false
        }
