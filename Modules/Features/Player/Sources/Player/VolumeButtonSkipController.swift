...
public final class VolumeButtonSkipController {
    ...
    public func configure(volumeController: SystemVolumeController = .shared) {
        ...
        latestObservedVolume = Float(volumeController.volume)
        previousObservedVolume = Float(volumeController.volume)
    ...
    private func handleVolumeButtonEvent(
        ...
        let mappedDirection: Direction = switch direction {
        case .up:
            .up
        case .down:
            .down
    ...
    private func ingestHardwareEvent(direction: Direction, baselineHint: Float?) {
        ...
        if var session {
            ...
            session = makeSession(direction: direction, baselineVolume: baselineVolume, at: now)
        }
    ...
    private func ensureProactiveEdgeReserveIfNeeded() {
        ...
        let currentVolume = Float(volumeController.volume)
        let reserveTarget: Float? = if currentVolume <= Tuning.edgeBoundaryEpsilon {
            Tuning.edgeReserve
        } else if currentVolume >= 1 - Tuning.edgeBoundaryEpsilon {
            1 - Tuning.edgeReserve
        } else {
            nil
        }
