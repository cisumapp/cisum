...
public final class YouTubePlaylistImportService {
    enum PlaylistImportError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .emptyQuery:
                "Enter a playlist name to search."
            case .invalidPlaylistLink:
                "Enter a valid YouTube playlist link or playlist ID."
            case .playlistNotFound:
                "Unable to find that playlist on YouTube."
            case .playlistHasNoTracks:
                "The playlist was found but has no importable tracks."
            }
        }
    }
...
private extension YouTubePlaylistImportService {
    ...
    private func importPlaylist(
        ...
        let existingPlaylist = await playlistStore.playlistSnapshot(
    ...
    private func fetchTracksFromMainPlaylist(playlistID: String, maxPages: Int = 12) async throws -> [ImportedTrackPayload]? {
        ...
        var payloads = firstPage.items.compactMap { video -> ImportedTrackPayload? in
            return makePayload(from: video)
        }
    ...
    private func fetchTracksFromMusicPlaylist(playlistID: String) async throws -> [ImportedTrackPayload]? {
        ...
        return songs.compactMap(makePayload(from:))
    }
    ...
    private func parsePlaylistLink(_ rawValue: String) throws -> ParsedPlaylistLink {
        ...
        let normalizedURLString: String = if trimmed.hasPrefix("http://") || trimmed.hasPrefix("https://") {
            trimmed
        } else {
            "https://\(trimmed)"
        }
